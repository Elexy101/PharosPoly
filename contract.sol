...
contract SoloMonopoly {
    ...
    string public constant name = "Pharos Monopoly";
    string public constant symbol = "PHAMONO";
    ...
    struct Tile {
        ...
        bytes12 name; // Packed storage for names
    }
    ...
    function _initializeBoard() private {
        // Packed tile initialization to save gas
        board[1] = Tile(TileType.PROFIT, 50, bytes12("Airbnb Boost"));
        board[2] = Tile(TileType.LOSS, -20, bytes12("Lost Wallet"));
        board[3] = Tile(TileType.NEUTRAL, 0, bytes12("Park"));
        board[4] = Tile(TileType.PROFIT, 150, bytes12("Crypto Jack"));
        board[5] = Tile(TileType.LOSS, -50, bytes12("Car Repair"));
        board[6] = Tile(TileType.PROFIT, 75, bytes12("Freelance"));
        board[7] = Tile(TileType.LOSS, -30, bytes12("Stolen Phn"));
        board[8] = Tile(TileType.PROFIT, 30, bytes12("Relax Zone"));
        board[9] = Tile(TileType.PROFIT, 80, bytes12("E-Commerce"));
        board[10] = Tile(TileType.LOSS, -40, bytes12("Overdue Rt"));
        board[11] = Tile(TileType.PROFIT, 100, bytes12("Angel Inv"));
        board[12] = Tile(TileType.LOSS, -25, bytes12("Bad Trade"));
        board[13] = Tile(TileType.PROFIT, 30, bytes12("Gift Bonus"));
        board[14] = Tile(TileType.LOSS, -10, bytes12("Late Fee"));
        board[15] = Tile(TileType.PROFIT, 50, bytes12("Chill Spot"));
    }
